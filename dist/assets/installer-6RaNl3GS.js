import{c as H,b as l,j as e,f as U,T as g,C as Ne,B as Fe,D as Nt,E as xt,z as At,F as Me,u as bt,w as It,k as ne,S as Ce,a as he,l as ye,i as Pe,h as ke,K as tt,L as ft,A as _t}from"./index-DBtsGOZR.js";/**
...
`,ve={};let q=null;function Oe(){const t=ve==null?void 0:ve.VITE_API_URL;return t&&typeof t=="string"&&t.trim()?t.trim().replace(/\/+$/,""):""}function k(t){return Oe()+t}function Xt(t,r){const s=t.trimStart();return(r??"").includes("text/html")||s.startsWith("<!doctype")||s.startsWith("<html")}const Wt="The /api/install/* endpoints returned an HTML page instead of JSON — the installer is not talking to a Recura server. "+(Oe()?`Check that the Recura server is running at ${Oe()} and that VITE_API_URL points at its HTTPS origin (no trailing slash).`:"Set the VITE_API_URL build environment variable to the HTTPS origin of your hosted Recura server (e.g. https://your-server.onrender.com).");async function Te(t,r){const s=await t.text();if(!s)return{};if(Xt(s,t.headers.get("content-type")))throw new Error(`${Wt} (${r} returned HTML with status ${t.status}.)`);try{return JSON.parse(s)}catch{throw new Error(`Unexpected server response (${t.status}). Reload the page and try again.`)}}async function Gt(){if(q)return q;const t=await fetch(k("/api/csrf"),{method:"GET",credentials:"include"});if(!t.ok)throw new Error(`Could not reach the Recura server at ${k("/api/csrf")}. Is it running?`);const r=await Te(t,"/api/csrf");if(q=(r==null?void 0:r.csrfToken)||null,!q)throw new Error("The Recura server did not issue a security token. Reload the page and try again.");return q}async function $(t,r,s){const o={"Content-Type":"application/json","X-CSRF-Token":await Gt()};s&&(o.Authorization=`Bearer ${s}`);let h;try{h=await fetch(k(t),{method:"POST",headers:o,credentials:"include",body:JSON.stringify(r??{})})}catch{throw new Error(`Could not reach the Recura server at ${k(t)}. Is it running?`)}const L=await Te(h,t),i=L;if(!h.ok||i.ok===!1)throw i.code==="CSRF"&&(q=null),new Error(i.message||`Request failed (${h.status}).`);return L}const O={getStatus:async()=>{const t=await fetch(k("/api/install/status"),{credentials:"include"});if(!t.ok)throw new Error(`Could not reach the Recura server at ${k("/api/install/status")}. Is it running?`);return await Te(t,"/api/install/status")},getDbPresets:async()=>{const t=await fetch(k("/api/install/presets"),{credentials:"include"});return t.ok?await Te(t,"/api/install/presets"):{ok:!1,presets:[]}},testConnection:t=>$("/api/install/test-connection",{database:t}),startInstall:(t,r,s,m,o=!1)=>$("/api/install/start",{database:t,dbState:r,consent:s,resume:m,force:o}),migrate:t=>$("/api/install/migrate",{},t),createAdmin:(t,r)=>$("/api/install/admin",r,t),verify:t=>$("/api/install/verify",{},t),complete:t=>$("/api/install/complete",{},t)},Ye=Object.assign({"../../server/migrations/001_initial_schema.sql":yt,"../../server/migrations/002_default_whatsapp_templates.sql":jt,"../../server/migrations/003_order_number_backfill.sql":wt,"../../server/migrations/004_mobile_push_tables.sql":Dt,"../../server/migrations/005_widen_pin_code_encrypted.sql":Ft,"../../server/migrations/006_mobile_authentication.sql":Mt,"../../server/migrations/007_pre_request_hook.sql":Pt,"../../server/migrations/008_pair_device_rpc.sql":kt,"../../server/migrations/009_generic_products.sql":Ht}),Vt=Object.keys(Ye).sort().map(t=>Ye[t]).join(`
